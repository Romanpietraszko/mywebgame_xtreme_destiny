...
// --- ŁADOWANIE TWOICH AUTORSKICH POSTACI I ASYSTENTA ---
const skins = {
    standard: new Image(),
    ninja: new Image(),
...
function drawProArmor(x, y, sc, tier, strengthLvl) {
    ...
    if (strengthLvl >= 100) {
        ...
        if (!window.isMobile) { ctx.shadowBlur = 10 * sc; ctx.shadowColor = '#9b59b6'; }
...
function drawDynamicArmor(e, x, y, sc, tier) {
    ...
    let baseColor = '#bdc3c7'; 
    let trimColor = '#7f8c8d'; 
    let gemColor = null;
    ...
    if (tier >= 3) {
        ...
        if (skinType === 'ninja') {
            ...
            if (!window.isMobile) { ctx.shadowColor = crownColor; ctx.shadowBlur = 20; }
            ctx.beginPath(); 
...
function drawStickman(e, x, y, sc, safe, kingId) {
    ...
    if (e.aura && e.aura.time > 0) { 
        ...
        if (!window.isMobile) { ctx.shadowBlur = 20; ctx.shadowColor = e.aura.color; }
        ctx.stroke(); 
        ctx.restore(); 
    }
